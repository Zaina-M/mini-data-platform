...
from utils.logging_config import get_logger, setup_logging, PipelineLogger
        ...
        for file_name in csv_files:
            try:
                ...
                if result.is_valid:
                    ...
                    log.warning(
                        f"File {file_name} failed validation: "
        ...
        for file_name in valid_files:
            try:
                ...
                if not validation_result.is_valid:
                    log.warning(
                        f"Cleaned data from {file_name} failed post-clean validation"
                    )
                ...
                for col in df_serializable.select_dtypes(include=['datetime64', 'datetime64[ns]']).columns:
                ...
                log.info(
                    f"Cleaned {file_name}: {original_count} -> "
                    f"{len(df_validated)} rows"
                )
...
def compute_totals(**context) -> List[Dict[str, Any]]:
    ...
    with PipelineLogger("compute_totals", **context) as log:
        ...
        for item in cleaned_data:
            ...
            validation_result, df_validated = validator.validate_enriched_data(
                df_enriched
            )
...
def generate_summary(**context) -> Dict[str, Any]:
    ...
    with PipelineLogger("generate_summary", **context) as log:
        ...
        archive_result = ti.xcom_pull(task_ids="archive_processed_files", key="archive_result") or {}
