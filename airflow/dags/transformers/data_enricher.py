...
class DataEnricher:
    ...
    def enrich(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, EnrichmentResult]:
        ...
        total_revenue = float(df_enriched["total_amount"].sum()) if "total_amount" in df_enriched else 0
        avg_order_value = float(df_enriched["total_amount"].mean()) if "total_amount" in df_enriched else 0
    ...
    def _add_ingestion_timestamp(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(f"Added ingestion_timestamp column")
        return df
