...
class RawSalesSchema(pa.DataFrameModel):
    ...
    order_id: Series[str] = pa.Field(
        nullable=False, coerce=True, description="Unique order identifier"
    )
    product_name: Series[str] = pa.Field(
        nullable=True, coerce=True, description="Name of the product"
    )
    quantity: Series[str] = pa.Field(
        nullable=True, coerce=True, description="Quantity ordered (may be string initially)"
    )
    unit_price: Series[str] = pa.Field(
        nullable=True, coerce=True, description="Price per unit (may be string initially)"
    )
    order_date: Series[str] = pa.Field(nullable=True, coerce=True, description="Date of order")
    customer_id: Series[str] = pa.Field(
        nullable=True, coerce=True, description="Customer identifier"
    )
    country: Series[str] = pa.Field(nullable=True, coerce=True, description="Customer country")
...
class CleanedSalesSchema(pa.DataFrameModel):
    ...
    quantity: Series[int] = pa.Field(
        nullable=False, ge=1, le=10000, description="Quantity ordered (positive integer)"
    )
    unit_price: Series[float] = pa.Field(
        nullable=False, gt=0, le=100000, description="Price per unit (positive decimal)"
    )
    order_date: Series[pd.Timestamp] = pa.Field(
        nullable=False, description="Date of order (datetime)"
    )
    customer_id: Series[str] = pa.Field(
        nullable=False,
    ...
    country: Series[str] = pa.Field(
        nullable=False, isin=VALID_COUNTRIES, description="Customer country from valid list"
    )
...
class EnrichedSalesSchema(pa.DataFrameModel):
    ...
    quantity: Series[int] = pa.Field(nullable=False, ge=1, le=10000, description="Quantity ordered")
    unit_price: Series[float] = pa.Field(
        nullable=False, gt=0, le=100000, description="Price per unit"
    )
    order_date: Series[pd.Timestamp] = pa.Field(nullable=False, description="Date of order")
    customer_id: Series[str] = pa.Field(
        nullable=False, str_matches=r"^(CUST-\d{5}|UNKNOWN)$", description="Customer identifier"
    )
    country: Series[str] = pa.Field(
        nullable=False, isin=VALID_COUNTRIES, description="Customer country"
    )
    total_amount: Series[float] = pa.Field(
        nullable=False, gt=0, description="Calculated total (quantity * unit_price)"
    )
    ingestion_timestamp: Series[pd.Timestamp] = pa.Field(
        nullable=False, description="Timestamp when record was processed"
    )
...
def validate_dataframe(
    df: pd.DataFrame, schema: type, lazy: bool = True
