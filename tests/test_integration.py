...
class TestEndToEndPipeline:
    ...
    def test_complete_pipeline_flow(self, sample_raw_data):
        ...
        clean_result, validated_df = validator.validate_cleaned_data(
            cleaned_df, original_count=len(sample_raw_data)
        )
    ...
    def test_pipeline_with_data_quality_issues(self, sample_raw_data_with_errors):
        ...
        validator.validate_raw_data(sample_raw_data_with_errors, "errors.csv")
        ...
        if len(cleaned_df) > 0:
            ...
            clean_result, validated_df = validator.validate_cleaned_data(
                cleaned_df, original_count=len(sample_raw_data_with_errors)
            )
